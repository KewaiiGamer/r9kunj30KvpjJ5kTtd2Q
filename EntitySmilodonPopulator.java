...
public class EntitySmilodonPopulator extends EntityTameable implements IJumpingMount {
    ...
    private static final DataParameter<Boolean> BEGGING = EntityDataManager.<Boolean>createKey(EntitySmilodonPopulator.class, DataSerializers.BOOLEAN);
    private static final DataParameter<Float> DATA_HEALTH_ID = EntityDataManager.<Float>createKey(EntitySmilodonPopulator.class, DataSerializers.FLOAT);
    private static final DataParameter<Integer> COLLAR_COLOR = EntityDataManager.<Integer>createKey(EntitySmilodonPopulator.class, DataSerializers.VARINT);
...
//      ((PathNavigateGround) this.getNavigator()).setAvoidsWater(true);
    ...
    private void setHorseWatchableBoolean(int x, boolean saddled)
    {
        byte b0 = ((Byte)this.dataManager.get(STATUS)).byteValue();

        if (saddled)
        {
            this.dataManager.set(STATUS, Byte.valueOf((byte)(b0 | x)));
        }
        else
        {
            this.dataManager.set(STATUS, Byte.valueOf((byte)(b0 & ~x)));
 ...
 /*   @Override
    ...
    public boolean shouldAttackEntity(EntityLivingBase attacked, EntityLivingBase attacker) {
        if (!(attacked instanceof EntityCreeper) && !(attacked instanceof EntityGhast)) {
            if (attacked instanceof EntitySmilodonPopulator) {
                EntitySmilodonPopulator entitySmilodonPopulator = (EntitySmilodonPopulator) attacked;

                if (entitySmilodonPopulator.isTamed() && entitySmilodonPopulator.getOwner() == attacker) {
                    return false;
                }
            }
            return attacked instanceof EntityPlayer && attacker instanceof EntityPlayer && !((EntityPlayer) attacker).canAttackPlayer((EntityPlayer) attacked) ? false : !(attacked instanceof EntitySmilodonPopulator) || !((EntitySmilodonPopulator) attacked).isTamed();
        } else {
            return false;
        }
    }
